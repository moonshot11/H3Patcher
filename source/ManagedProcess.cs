...
namespace H3Patcher
{
    public class ManagedProcess : IDisposable
    {
        ...
        public ManagedProcess(string procName, bool scan=false)
        {
#if DEBUG
            if (scan)
            ...
            do
            {
                foundProcs = Process.GetProcessesByName(procName);
            } while (foundProcs.Length < 1 && scan);
        ...
        public void WriteNops(long writeAddr, uint nSize)
        {
            byte[] data = new byte[nSize];
            for (uint i = 0; i < data.Length; i++)
                data[i] = 0x90;
            Write(writeAddr, data);
        }
        ...
        {
            if (!Valid)
                throw new Exception("ManagedProcess not valid - could not read");
